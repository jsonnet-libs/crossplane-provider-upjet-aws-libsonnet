{
  ...
  '#':: d.pkg(name='bucket', url='', help='"Bucket is the Schema for the Buckets API. Provides a S3 bucket resource."'),
  ...
  spec: {
    ...
    forProvider: {
      '#withForceDestroy':: d.fn(help='"Boolean that indicates all objects (including any locked objects) should be deleted from the bucket when the bucket is destroyed so that the bucket can be destroyed without error. These objects are not recoverable. This only deletes objects when the bucket is destroyed, not when setting this parameter to true. If setting this field in the same operation that would require replacing the bucket or destroying the bucket, this flag will not work."', args=[d.arg(name='forceDestroy', type=d.T.boolean)]),
      withForceDestroy(forceDestroy): { spec+: { forProvider+: { forceDestroy: forceDestroy } } },
      '#withObjectLockEnabled':: d.fn(help='"Indicates whether this bucket has an Object Lock configuration enabled. Valid values are true or false. This argument is not supported in all regions or partitions."', args=[d.arg(name='objectLockEnabled', type=d.T.boolean)]),
