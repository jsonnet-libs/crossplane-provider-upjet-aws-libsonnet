{
  ...
  spec: {
    ...
    forProvider: {
      ...
      '#withFilter':: d.fn(help="\"Bucket filter. The configuration only includes objects that meet the filter's criteria (documented below).\"", args=[d.arg(name='filter', type=d.T.array)]),
      withFilter(filter): { spec+: { forProvider+: { filter: if std.isArray(v=filter) then filter else [filter] } } },
      '#withFilterMixin':: d.fn(help="\"Bucket filter. The configuration only includes objects that meet the filter's criteria (documented below).\"\n\n**Note:** This function appends passed data to existing values", args=[d.arg(name='filter', type=d.T.array)]),
